...
qq.maps.event.addDomListener(btnSearch, 'click', function () {
    ...
    query_city = "深圳市";
    url = encodeURI("http://apis.map.qq.com/ws/place/v1/search?keyword=" + value + "&boundary=region(" + query_city + ",0)&page_size=9&page_index=1&key=6NIBZ-YGFK4-P2DUR-X7NU5-246HF-HXBIA&output=jsonp&&callback=?");
...
city.onclick = function (e) {
    ...
    if (target.className == "city_name") {
        ...
        dsf_dsf = target.innerHTML;

        url2 = encodeURI("http://apis.map.qq.com/ws/geocoder/v1/?region=" + dsf_dsf + "&address=" + dsf_dsf + "&key=K76BZ-W3O2Q-RFL5S-GXOPR-3ARIT-6KFE5&output=jsonp&&callback=?");
...
$(".search_t").autocomplete({
    source: function (request, response) {
        url4 = encodeURI("http://apis.map.qq.com/ws/place/v1/suggestion/?keyword=" + request.term + "&region=" + dsf_dsf + "&key=6NIBZ-YGFK4-P2DUR-X7NU5-246HF-HXBIA&output=jsonp&&callback=?");
